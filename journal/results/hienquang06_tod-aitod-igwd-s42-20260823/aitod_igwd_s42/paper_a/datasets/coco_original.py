...
class CocoOriginalDataset(Dataset):
    ...
    def _load_image(self, image_info: dict) -> Image.Image:
        ...
        # Robust fallback: Search up the directory tree for raw_name
        current = self.image_root
        for _ in range(4):
            if current.exists():
                found = list(current.rglob(raw_name))
        ...
        # Global input fallback for Kaggle environments
        kaggle_input = Path("/kaggle/input")
        if kaggle_input.exists():
            found = list(kaggle_input.rglob(raw_name))
            if found and found[0].is_file():
                with Image.open(found[0]) as source:
                    return source.convert("RGB")
