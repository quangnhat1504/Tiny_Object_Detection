...
def train_aitod(
    ...
    if data_root is None:
        ...
        if data_root is None and Path("/kaggle/input").exists():
            for c in Path("/kaggle/input").glob("**/AI-TOD"):
                if (c / "annotations/train.json").exists():
                    data_root = c
                    break
        if data_root is None:
    ...
    if not val_img_dir.exists():
        val_img_dir = data_root / "images"
    if not val_img_dir.exists():
        val_img_dir = data_root

    train_ann_file = data_root / "annotations/train.json"
    if not train_ann_file.exists():
        train_ann_file = data_root / "annotations/aitod_train.json"
    val_ann_file = data_root / "annotations/val.json"
    if not val_ann_file.exists():
        val_ann_file = data_root / "annotations/aitod_val.json"
...
def main():
    ...
    parser.add_argument("--metric", type=str, default="h_wiou",
                        choices=["standard", "nwd", "igwd", "rfla", "alw_canonical", "sa_alw_canonical", "h_wiou"])
