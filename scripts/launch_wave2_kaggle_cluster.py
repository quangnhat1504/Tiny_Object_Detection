"""
Launcher for Wave 2 Experiments on AI-TOD-v2 (Kaggle GPU Cluster):
1. Dynamic Uncertainty Homotopy (DU-HWIoU) on quangnhtng
2. Task-Aligned Quality Focal Loss (QFL) + H-WIoU on trieuvo123
"""
...
PROFILE_ROOT = Path(r"C:\tmp\tod_kaggle_wave2_profiles")
PROFILE_ROOT.mkdir(parents=True, exist_ok=True)
RUNTIME_DIR = ROOT / ".runtime/kaggle_aitod_wave2"
...
WAVE2_EXPS = [
    {
        "tag": "aitod_du_hwiou_s42_proposed",
        "slug": "tod-aitod-du-hwiou-s42-proposed",
        "title": "tod-aitod-du-hwiou-s42-proposed",
        "account": "quangnhtng",
        "cred": CREDS_DIR / "kaggle (6).json",
        "cmd": "python scripts/train_frcnn_aitod.py --metric du_hwiou --placement la_loss --box-loss h_wiou --h-wiou-sigma-0 8.0 --batch-size 4 --seed 42 --epochs 12 --tag du_hwiou_s42",
    },
    {
        "tag": "aitod_qfl_hwiou_s42_proposed",
        "slug": "tod-aitod-qfl-hwiou-s42-proposed",
        "title": "tod-aitod-qfl-hwiou-s42-proposed",
        "account": "trieuvo123",
        "cred": CREDS_DIR / "kaggle (10).json",
        "cmd": "python scripts/train_frcnn_aitod.py --metric h_wiou --placement la_loss --box-loss h_wiou --h-wiou-sigma-0 8.0 --use-quality-focal --quality-focal-beta 2.0 --batch-size 4 --seed 42 --epochs 12 --tag qfl_hwiou_s42",
    },
]
...
def main():
    print("=" * 80)
    print("   LAUNCHING WAVE 2 EXPERIMENTS (DU-HWIoU & QFL+HWIoU) ON KAGGLE GPU CLUSTER   ")
    print("=" * 80)

    for exp in WAVE2_EXPS:
        account = exp["account"]
        tag = exp["tag"]
        slug = exp["slug"]
        title = exp["title"]
    ...
    print("\n" + "=" * 80)
    print("   ALL WAVE 2 EXPERIMENTS DISPATCHED SUCCESSFULLY!   ")
    print("=" * 80)
