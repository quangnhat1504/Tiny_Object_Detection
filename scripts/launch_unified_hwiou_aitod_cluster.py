"""
Master Multi-Agent Launcher for Unified H-WIoU Assigner + Loss Experiments on AI-TOD-v2 (Kaggle GPU Cluster).
"""
...
PROFILE_ROOT = Path(r"C:\tmp\tod_kaggle_unified_profiles")
PROFILE_ROOT.mkdir(parents=True, exist_ok=True)
RUNTIME_DIR = ROOT / ".runtime/kaggle_aitod_unified"
...
UNIFIED_EXPS = [
    {
        "tag": "aitod_hwiou_unified_sig8_s42",
        "slug": "tod-aitod-hwiou-unified-sig8-s42",
        "title": "TOD AI-TOD H-WIoU Unified Sigma8 S42 Proposed",
        "account": "hngtrngtn",
        "cred": CREDS_DIR / "kaggle (7).json",
        "cmd": "python scripts/train_frcnn_aitod.py --metric h_wiou --placement h_wiou --box-loss h_wiou --h-wiou-sigma-0 8.0 --batch-size 4 --seed 42 --epochs 12 --eval-interval 3 --tag hwiou_unified_sig8_s42",
    },
    {
        "tag": "aitod_hwiou_unified_sig6_s42",
        "slug": "tod-aitod-hwiou-unified-sig6-s42",
        "title": "TOD AI-TOD H-WIoU Unified Sigma6 S42 Ablation",
        "account": "luongsythanh",
        "cred": CREDS_DIR / "kaggle (8).json",
        "cmd": "python scripts/train_frcnn_aitod.py --metric h_wiou --placement h_wiou --box-loss h_wiou --h-wiou-sigma-0 6.0 --batch-size 4 --seed 42 --epochs 12 --eval-interval 3 --tag hwiou_unified_sig6_s42",
    },
    {
        "tag": "aitod_hwiou_unified_sig10_s42",
        "slug": "tod-aitod-hwiou-unified-sig10-s42",
        "title": "TOD AI-TOD H-WIoU Unified Sigma10 S42 Ablation",
        "account": "pptlyn11",
        "cred": CREDS_DIR / "kaggle (9).json",
        "cmd": "python scripts/train_frcnn_aitod.py --metric h_wiou --placement h_wiou --box-loss h_wiou --h-wiou-sigma-0 10.0 --batch-size 4 --seed 42 --epochs 12 --eval-interval 3 --tag hwiou_unified_sig10_s42",
    },
]

def main():
    print("=" * 80)
    print("      LAUNCHING UNIFIED H-WIOU BENCHMARKS ON KAGGLE GPU CLUSTER      ")
    print("=" * 80)

    for exp in UNIFIED_EXPS:
        account = exp["account"]
        tag = exp["tag"]
        slug = exp["slug"]
        title = exp["title"]
