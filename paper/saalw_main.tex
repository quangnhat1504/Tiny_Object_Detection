...
%  Springer proceedings submission format for ICogSys 2026.
%  Compile from paper/: pdflatex saalw_main → bibtex saalw_main → pdflatex ×2
% =============================================================================
...
\titlerunning{SA-ALW for Tiny Object Detection}

% ICogSys 2026 uses double-blind review. Do not add authors, affiliations,
% emails, acknowledgements, or identifying repository links before acceptance.
...
\keywords{Tiny object detection \and bounding-box metric \and Wasserstein
distance \and label assignment \and scale-adaptive metrics \and
...
SA-ALW introduces two scale-adaptive mechanisms. Let $s=\sqrt{w_t h_t}$ be
the geometric mean dimension of the ground-truth box (a stable, monotonic
proxy for detection difficulty).
The scale bounds $s_{\text{min}}$ and $s_{\text{max}}$ are set to the 10th
and 90th percentiles of the training-set $\sqrt{wh}$ distribution, making
...
Figure~\ref{fig:saalw-overview} summarizes how the scale statistic controls
both adaptive schedules before the resulting metric is used by the detector.
\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  ...
  >=Latex,
  proc/.style={draw, rounded corners, align=center, minimum height=9mm,
    text width=25mm, font=\footnotesize},
  metric/.style={draw, rounded corners, align=center, minimum height=9mm,
    text width=28mm, font=\footnotesize, fill=gray!8},
  output/.style={draw, rounded corners, align=center, minimum height=9mm,
    text width=28mm, font=\footnotesize},
  flow/.style={-{Latex[length=1.5mm]}, thick}
]
  \node[proc] (boxes) {Target and candidate\\boxes $(x,y,w,h)$};
  \node[proc, right=of boxes] (scale) {Target scale\\$s=\sqrt{w_t h_t}$};
  \node[proc, right=of scale] (schedule) {Percentile\\schedule\\$s_{\min},s_{\max}$};
  \node[metric, right=of schedule] (adaptive) {Adaptive parameters\\$\beta(s)$,\\$w_{\mathrm{pos}}(s)$};
  \node[metric, right=of adaptive] (distance) {SA-ALW\\distance and\\similarity};
  \node[output, above right=of distance] (assignment) {RFLA-HLA\\label assignment};
  \node[output, below right=of distance] (regression) {Bounding-box\\regression loss};
  \draw[flow] (boxes) -- (scale);
  \draw[flow] (scale) -- (schedule);
  \draw[flow] (schedule) -- (adaptive);
  \draw[flow] (adaptive) -- (distance);
  \draw[flow] (distance) -- (assignment);
  \draw[flow] (distance) -- (regression);
\end{tikzpicture}%
}
\caption{SA-ALW data flow. The target scale determines the temperature and
position-weight schedules, which parameterize the same anisotropic
log-Wasserstein metric for label assignment and box regression.}
\label{fig:saalw-overview}
\end{figure}
