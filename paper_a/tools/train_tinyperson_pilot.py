...
def build_method_model(method: str, schedule: dict) -> torch.nn.Module:
    ...
    if method == "h_wiou":
        # Proposed Homotopy Wasserstein-IoU (Continuous Scale Homotopy, sigma_0=8.0px)
        return build_model(
            metric_fn=get_metric_fn("h_wiou"), placement="la_loss",
            box_loss_type="metric", **common_kwargs,
        )
    if method == "rfla":
...
def build_datasets(args, transform_seed: int):
    candidates = [
        args.data_root / "erase_with_uncertain_dataset" / "train",
        args.data_root / "tiny_set" / "erase_with_uncertain_dataset" / "train",
